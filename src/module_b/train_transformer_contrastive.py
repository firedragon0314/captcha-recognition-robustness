...
def get_lr(epoch):
    ...
    Epoch 1-50: 0.001
    Epoch 51-100: 0.0001
    Epoch 101-150: 0.00001
    Epoch 151-200: 0.000001
    """
    if epoch <= 50:
        return 1e-3
    elif epoch <= 100:
...
def load_pretrained_cnn(model, checkpoint_path, device):
    """
    Load CNN encoder weights from a B-2 contrastive pretraining checkpoint
    into CNNTransformerCTC.cnn.

    The checkpoint must contain 'encoder_state_dict' (produced by
    pretrain_b2_contrastive_transformer.py). CNNTransformerCTC.cnn and
    CaptchaCNNEncoder.cnn share the same architecture when d_model=128,
    so the weights transfer directly.
    """
    checkpoint = torch.load(checkpoint_path, map_location=device)

    if "encoder_state_dict" not in checkpoint:
        raise KeyError(
            f"Checkpoint '{checkpoint_path}' does not contain 'encoder_state_dict'. "
            "Make sure it was produced by pretrain_b2_contrastive_transformer.py."
        )

    model.cnn.load_state_dict(checkpoint["encoder_state_dict"], strict=True)
    print(f"Pretrained CNN encoder loaded from: {checkpoint_path}")
...
def train_one_epoch(model, data_loader, criterion, optimizer, device):
    ...
    progress = tqdm(
        data_loader,
        desc="B-2 Contrastive Pretrained Transformer Fine-tuning",
        leave=False
    )
...
def main():
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Path to the B-2 contrastive pretraining checkpoint "
            "(output of pretrain_b2_contrastive_transformer.py). "
            "Must contain 'encoder_state_dict'."
        )
    )
    ...
    parser.add_argument(
        "--experiment_name",
        type=str,
        default="b2_contrastive_pretrained_transformer_ctc"
    )
    ...
    config = {
        ...
        "training_method": "Unsupervised Contrastive Pretraining + Supervised Fine-tuning",
        "task": "sequence_based_captcha_recognition",
        "description": (
            "B-2 unsupervised contrastive learning. "
            "Stage 1: the CNN backbone is pretrained with NT-Xent contrastive loss "
            "(no labels used). "
            "Stage 2: the pretrained CNN weights are loaded into the full "
            "CNN + Transformer Encoder + CTC model and fine-tuned with supervised "
            "CTC loss on labeled CAPTCHA data."
        ),
        "charset": CHARS,
        ...
        "training": {
            "stage_1": {
                "method": "Unsupervised Contrastive Learning (NT-Xent)",
                "pretrained_encoder": args.pretrained_encoder,
                "label_usage": "No labels used in Stage 1."
            },
            "stage_2": {
                "method": "Supervised Fine-tuning with CTC Loss",
                "epochs": args.epochs,
    ...
    print("=" * 70)
    print("B-2 Experiment 2: Contrastive Pretrained CNN + Transformer Encoder + CTC")
    ...
    print(f"Pretrained encoder: {args.pretrained_encoder or 'None (training from scratch)'}")
    print("=" * 70)
    ...
    if args.pretrained_encoder:
        load_pretrained_cnn(model, args.pretrained_encoder, device)
    ...
    print("\nB-2 contrastive pretrained Transformer training finished.")

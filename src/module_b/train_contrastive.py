...
def plot_loss(exp_dir):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(plot_dir, "contrastive_loss.png"), bbox_inches="tight")
    plt.savefig(os.path.join(plot_dir, "contrastive_loss.svg"), bbox_inches="tight")
    plt.close()
...
def train_one_epoch(model, data_loader, optimizer, device, temperature):
    ...
    progress = tqdm(data_loader, desc="Contrastive Training", leave=False)
...
def main():
    ...
    config = {
        ...
        "stage": "CNN encoder pretraining",
        "description": "Two augmented views of the same CAPTCHA image are treated as positive pairs; different images in the batch are treated as negative pairs.",
        "dataset": {
    ...
    print("=" * 70)
    print("B-1 Training Method 2: Unsupervised Contrastive Learning")
    print("=" * 70)
